...
fn play_sound(audio_subsystem: &mut sdl2::AudioSubsystem)
    -> std::result::Result<sdl2::audio::AudioDevice<sound::Sound>, std::string::String>
{
    //use std::borrow::Cow;
    ...
    let audio_device = audio_subsystem.open_playback(None, &desired_spec, |_spec| {
        // Note: experimenting with using raw .wav
        ...
        //let _data = cvt.convert(wav.buffer().to_vec());
        let file = std::path::Path::new("/Users/plot/pg/pacman/assets/sounds/pacman-begin.wav");
        let sound_asset = sound::SoundAsset::from_wav(file);

        // initialize the audio callback
        sound::Sound {
            data: sound_asset.data,
            volume: 0.25,
            pos: 0,
        }
    })?;
